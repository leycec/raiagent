...
src_prepare() {
	...
	#
	# Additionally, apply the following 0.9c-specific patches:
	#
	# * Coerce use of the new "ncursesw6-config" linker utility rather than the
	#   long-obsolete "ncursesw5-config" linker utility, which no longer exists
	#   on modern ncurses installations.
	sed -i\
		...
		-e 's~\bncursesw5-config\b~ncursesw6-config~'\
