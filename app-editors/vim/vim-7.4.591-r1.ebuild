...
# This temporary fork of the official Vim ebuild installs support for all
# enabled Python implementations as specified by ${PYTHON_TARGETS}. By contrast,
# such ebuild only installs support for the single Python implementation
# specified by ${PYTHON_SINGLE_TARGET}. Follow this bug at:
#
#     https://bugs.gentoo.org/show_bug.cgi?id=537176
...
src_configure() {
	...
	if use minimal ; then
		...
		if use python ; then
			# Set list global ${MULTIBUILD_VARIANTS} to a list of all enabled
			# Python implementations (e.g., "python2_7", "python3_3").
			_python_obtain_impls

			case "${MULTIBUILD_VARIANTS[@]}" in
			*"python3"*)
				myconf+=( --enable-python3interp )
				export vi_cv_path_python3="${PYTHON}" ;;&  # continue checking
			*"python2"*)
